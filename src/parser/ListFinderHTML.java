...
public class ListFinderHTML {
	...
	public ArrayList<String> getNextList() {
		...
		if (prevtable) {
			...
			currenttablenumber++;
			if (tableheaderlist.size() > currenttablenumber)
				header = tableheaderlist.get(currenttablenumber);
		...
		if (indextable == -1 && indexol == -1 && indexul == -1 && indexdl == -1
				&& indexselect == -1) {
			currentpos = -1;
			return null;
		}
		...
		if (indexol == -1)
			indexol = Integer.MAX_VALUE;
		if (indexul == -1)
			indexul = Integer.MAX_VALUE;
		if (indexdl == -1)
			indexdl = Integer.MAX_VALUE;
		if (indexselect == -1)
			indexselect = Integer.MAX_VALUE;
		...
		if (indextable <= indexol && indextable <= indexul
			...
			prevtable = true;
			if (tableheaderlist.size() > 0) {
				header = tableheaderlist.get(0);
				return ProcessAndSend(tablelistlist.get(currenttablenumber));
			}
		...
		while (true) {
			...
			while (match.find()) {
				...
				if (match.group(1).equals("<th>"))
					...
					tablelistlist.get(i).add(match.group(2));
	...
	private void SetHeader() {
		...
		if (headerstart != -1
				&& !tempheader.substring(headerstart, headerstart + 3)
						.equalsIgnoreCase("<ht")
				&& !tempheader.substring(headerstart, headerstart + 3)
						.equalsIgnoreCase("<he")) {
			...
			if (headerstart == 0 || headerend == -1 || headerstart > headerend)
				return;
			// Make sure of the exact numbers here
			// System.out.println(tempheader.length() +" : "+ headerstart +
			// " : "+ headerend);
